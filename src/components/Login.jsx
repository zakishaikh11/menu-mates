...
const Login = () => {
    ...
    useEffect(() => {
        ...
        if (token) {
            try {
                const parsedToken = JSON.parse(token); // Check if it's a valid token format
                if (!parsedToken || typeof parsedToken !== 'string') {
                    localStorage.removeItem('token'); // Remove if it's not valid
                } else {
                    navigate('/');
                }
            } catch (error) {
                localStorage.removeItem('token'); // If parsing fails, remove it
            }
        }
    }, [navigate]);
