...
router.get('/', async (req, res) => {
  ...
  try {
    const products = await Product.find({vendor_id}).find(filter).sort(sortOption);
    res.json(products);
  } catch (error) {
...
router.get('/all-products', async (req, res) => {
  ...
  if (brand) filter.brand = brand;
  if (featured) filter.featured = featured === 'true';
  if (ratings) filter.ratings = { $gte: parseFloat(ratings) };
  ...
  else if (sort === 'best_sellers') sortOption.sales = -1;

  try {
    const products = await Product.find(filter).sort(sortOption); // No vendor_id filtering
    res.json(products);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
