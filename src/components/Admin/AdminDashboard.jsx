...
import { fetchAllOrders, fetchProducts, fetchOrdersByVendor, fetchProductsByVendor, fetchCustomersByVendor } from '../../api'; 
  ...
  useEffect(() => {
    const getAllOrdersAndProducts = async () => {
      try {
        ...
        console.log("Vendor ID from localStorage:", localStorage.getItem("vendor_id"));
        const orders = await fetchOrdersByVendor(vendorId);
        setOrders(orders);
        const products = await fetchProductsByVendor(vendorId);
        const customers = await fetchCustomersByVendor(vendorId);
        ...
        setSalesData({ labels: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'], data: salesByDay });
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <aside className="w-64 bg-gray-900 text-gray-100">
        ...
        <nav className="mt-10 space-y-2">
          ...
          <Link to="/admin/download-qr"><p className="block py-2.5 px-4 rounded hover:bg-gray-700">Download QR</p></Link>
        </nav>
      </aside>
    ...
    {recentProducts.map(product => (
      <tr key={product.product_id}>
        <td className="px-4 py-2 border text-center">
          ...
          alt={product.title} className="w-16 h-16 bg-white px-1 mx-auto object-contain border-2 border-gray-600 rounded-md" />
        </td>
