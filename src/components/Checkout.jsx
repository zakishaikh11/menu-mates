...
const Checkout = () => {
  ...
  const handlePayment = async (e) => {
    ...
    try {
      let paymentResponse = true; // Default true for COD (since no API call is needed)
  
      if (paymentMethod === 'Card') {
        paymentResponse = await addPayment(paymentData);
      }
  ...
  useEffect(() => {
    const fetchCart = async () => {
      ...
      const updatedCartItems = await Promise.all(
        cartData.items.map(async (item) => {
          const product = await fetchProductById(item.product_id);
          return { ...item, title: product.title, vendor_id: product.vendor_id };
        })
      );
      ...
      const calculatedTax = subtotal * 0.05; // 5% tax
      setTax(calculatedTax);
  
      setGrandTotal(subtotal + calculatedTax + 100); // Shipping cost ₹100
    };
  
    fetchCart();
  }, [navigate]); // Remove totalAmount from dependencies to avoid outdated calculations
        ...
        <form onSubmit={handlePayment} className="space-y-4">   
          ...
          {paymentMethod === 'Card' && (
            <>
              <div>
                <label className="block text-sm font-semibold mb-1">Card Number</label>
                <input type="text" className="w-full p-2 border border-gray-300 rounded" value={cardNumber} onChange={(e) => setCardNumber(e.target.value)} required />
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1">Expiry Date</label>
                <input type="text" className="w-full p-2 border border-gray-300 rounded" placeholder="MM/YY" value={expiryDate} onChange={(e) => setExpiryDate(e.target.value)} required />
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1">CVV</label>
                <input type="text" className="w-full p-2 border border-gray-300 rounded" value={cvv} onChange={(e) => setCvv(e.target.value)} required />
              </div>
            </>
          )}
