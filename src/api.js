...
export const fetchOrdersByVendor = async () => {
  try {
    ...
    if (!vendor_id) throw new Error('Vendor ID not found in localStorage');
    
    const response = await fetch(`${BASE_URL}/orders?vendor_id=${vendor_id}`);
...
export const fetchProductsByVendor = async () => {
  try {
    ...
    const response = await fetch(`${BASE_URL}/products?vendor_id=${vendor_id}`);
...
// Fetch customers by vendor ID
export const fetchCustomersByVendor = async () => {
  try {
    const vendor_id = localStorage.getItem('vendor_id');
    if (!vendor_id) throw new Error('Vendor ID not found in localStorage');
    
    const response = await fetch(`${BASE_URL}/customers?vendor_id=${vendor_id}`);
    if (!response.ok) throw new Error('Failed to fetch customers');
    
    return await response.json();
  } catch (error) {
    console.error('Error fetching customers by vendor:', error);
    throw error;
  }
};
